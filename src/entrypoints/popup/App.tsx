...
function IconBriefcase({ active }: { active: boolean }) {
  return (
    <svg
      className={`w-4 h-4 ${active ? 'text-brand' : 'text-brand-subtle'}`}
      fill="none"
      viewBox="0 0 24 24"
      strokeWidth={1.8}
      stroke="currentColor"
...
function App(): React.ReactElement {
  ...
  useEffect(() => {
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      ...
      const isJob = jobPatterns.some((pattern) => url.toLowerCase().includes(pattern));
      setIsJobPage(isJob);
    });
  }, []);
  ...
  return (
    <div
      ...
      style={{ width: 400, height: 580, minWidth: 400, minHeight: 580 }}
    >
      {/* Header */}
      <div className="bg-brand px-4 pt-4 pb-3 flex-shrink-0">
        <div className="flex items-center gap-2.5">
          ...
          <div className="flex-1">
            <h1 className="text-white text-sm leading-tight" style={{ fontWeight: 700 }}>
              AutoApply Copilot
            </h1>
            <p className="text-xs leading-tight" style={{ color: 'rgba(186,230,253,1)' }}>
              Smart job application assistant
            </p>
          </div>
        </div>

        {/* Quick Autofill Button - Shows only on job pages */}
        {isJobPage && (
          <button
            onClick={handleAutofill}
            className="mt-3 w-full flex items-center justify-center gap-2 bg-white text-brand hover:bg-blue-50 text-sm font-bold px-4 py-2.5 rounded-lg transition-all duration-150 cursor-pointer shadow-sm"
          >
            <svg
              className="w-4 h-4"
              fill="none"
      ...
      <div className="flex border-b border-brand-border bg-white flex-shrink-0" role="tablist">
        <TabButton
          ...
          icon={<IconBriefcase active={activeTab === 'tracker'} />}
        />
        <TabButton
          id="profile"
          label="Profile"
          ...
          icon={<IconUser active={activeTab === 'profile'} />}
        />
        <TabButton
          id="settings"
          label="Settings"
          ...
          icon={<IconCog active={activeTab === 'settings'} />}
        />
      </div>
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full px-6 text-center gap-5">
      ...
      <div>
        ...
        <p className="text-brand-muted text-xs leading-relaxed">
          Configure your AI API key, autofill preferences, and disabled sites from the settings
          page.
        </p>
      </div>
      <button
